...
const Login = () => {
  ...
  const handleCheck = async (id) => {
    try {
      const response = await fetch("http://ec2-13-126-194-20.ap-south-1.compute.amazonaws.com:4000/api/check-random-id", {
        ...
        body: JSON.stringify({ randomId: id, Account: "" }),
      });
  ...
  const handleRegister = async () => {
    await handleCheck(userReference_id);

    if (userReference_id.trim() === reference_id.trim()) {
      // Store auth token (example: from backend response)
      const authToken = "sampleAuthToken123"; // Replace with actual token from API
      ...
      <main className="flex flex-col items-center w-full px-4">
        ...
        <section className="white-glassmorphism text-base rounded-lg p-8 mt-8 w-full max-w-4xl flex flex-col md:flex-row justify-between items-center">
          <div className="text-center md:text-left">
            ...
            <p className="mt-2">
              Wallet is not a member of The Climate Crew. You can use this wallet to register as a new member.
            </p>
            <div className="mt-4">
          ...
          <img src={logo} alt="The Rich Crowd Icon" className="h-24 mt-4 md:mt-0 cursor-pointer" onClick={togglePopup} />
          {isOpen && (
              ...
              <div className="bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 p-8 rounded-lg shadow-lg text-white">
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="flex justify-end">
                    <button type="submit" className="bg-white text-black px-4 py-2 rounded mr-2">
                      Submit
                    </button>
                    <button type="button" className="bg-white text-black px-4 py-2 rounded" onClick={togglePopup}>
                      Close
                    </button>
                  </div>
                </form>
        ...
        <section className="text-center mt-12 w-full max-w-4xl">
          ...
          <p className="mt-2">
            Look up any The Climate Crew member account in preview mode. Enter ID or Temz address to preview a random account.
          </p>
          ...
          <div className="mt-4 flex flex-col md:flex-row justify-between">
            <div className="bg-gray-800 p-4 rounded-lg w-full md:w-1/2 mb-4 md:mb-0 md:mr-2">
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <span>{totalRegistrations * 100}</span>
              </div>
            </div>
